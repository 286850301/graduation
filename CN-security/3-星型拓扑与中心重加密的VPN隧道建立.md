...
在本练习中，你将在中心局和分支机构的受保护子网之间配置安全通信。安全交互是通过使用国内行业标准 GOST 和 IPsec 协议对流量进行加密和隧道化来实现的。受保护分支子网之间的安全通信是通过中心局的加密网关进行的。允许所有其他连接，但不会受到 IPsec 的保护。在此方案中，证书将用于身份验证。S-Terra CSP 开发的加密库将用作加密提供商。安全网关（或加密网关）——“S-Terra 网关”4.3 版。

## 基础设施要求

### 设备要求

1.1. 《С-Терра》网关设备必须已完成初始化  
...
1.1. 在总部部署：加密网关《С-Терра》（Hub1）和个人计算机（host_behind_hub1）  
1.2. 在分支机构部署：加密网关《С-Терра》（Spoke1和Spoke2）和个人计算机（host_behind_spoke1和host_behind_spoke2）  
